const Customer = require('../models');
const jwt = require('jsonwebtoken');

const createToken = async (customer) => {
  const { email, name } = customer;
  const token = await jwt.sign(
    { email, name },
    process.env.JWT_SECRET || 'secret_key',
  );
  return token;
};

const registerCustomer = async (req, res) => {
  const { name, email, password, age } = req.body;
  try {
    // mongoose model
    const newCustomer = new Customer({
      name,
      email,
      password,
      age,
    });

    const customer = await newCustomer.save();

    res.status(201).send(customer);
  } catch (e) {
    console.error('---Error registering customer ----', e);
    res.status(500).send({ message: 'Error registering new customer' });
  }
};

const loginCustomer = async (req, res) => {
  const { email, password } = req.body;
  try {
    const customer = await Customer.findOne({ email });
    if (!customer) {
      res.status(401).send({ message: 'Either email or password  is invalid' });
    }
    const match = await customer.isValidPassword(password);
    if (match) {
      const token = await createToken(customer);
      res.status(200).send(token);
    } else {
      res.status(401).send({ message: 'Either email or password  is invalid' });
    }
  } catch (e) {
    console.error('---Error login customer ----', e);
    res.status(403).send({ message: 'Invalid Login' });
  }
};

const fetchAllCustomers = async (req, res) => {
  try {
    const customers = await Customer.find({});
    res.status(200).send(customers);
  } catch (e) {
    console.error('---Error fetching customers ----', e);
    res
      .status(500)
      .send({ message: 'Error fetching all customer information' });
  }
};

const fetchCustomerById = async (req, res) => {
  const id = req.params.id;
  try {
    const customer = await Customer.findOne({ _id: id });
    res.status(200).send(customer);
  } catch (e) {
    console.error('---Error fetching specific customer specific ----', e);
    res
      .status(500)
      .send({ message: 'Error fetching specific customer information' });
  }
};

module.exports = {
  registerCustomer,
  loginCustomer,
  fetchAllCustomers,
  fetchCustomerById,
};
